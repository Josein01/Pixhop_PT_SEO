...
<Seo
  ...
  canonicalEn="https://pixshop.com/"
  canonicalPt="https://pixshop.com/pt/"
  ogImage="https://pixshop.com/og-image.jpg"
/>
...
const App: React.FC = () => {
  ...
  const handleGenerate = useCallback(async () => {
    ...
    if (!editHotspot) {
        setError(t('error.clickToSelect'));
        return;
    }
  ...
  const handleApplyFilter = useCallback(async (filterPrompt: string) => {
    if (!currentImage) {
      setError(t('error.noImageFilter'));
      return;
    }
  ...
  const handleApplyAdjustment = useCallback(async (adjustmentPrompt: string) => {
    if (!currentImage) {
      setError(t('error.noImageAdjust'));
      return;
    }
    ...
    if (error) {
       return (
           <div className="text-center animate-fade-in bg-red-500/10 border border-red-500/20 p-8 rounded-lg max-w-2xl mx-auto flex flex-col items-center gap-4">
            <h2 className="text-2xl font-bold text-red-300">{t('error.title')}</h2>
    ...
    return (
      <div className="w-full max-w-4xl mx-auto flex flex-col items-center gap-6 animate-fade-in">
            ...
            {activeTab === 'retouch' && (
                <div className="flex flex-col items-center gap-4">
                    <p className="text-md text-gray-400">
                        {editHotspot ? 'Great! Now describe your localized edit below.' : t('retouch.click')}
                    </p>
                    <form onSubmit={(e) => { e.preventDefault(); handleGenerate(); }} className="w-full flex items-center gap-2">
                        <input
                            ...
                            placeholder={editHotspot ? t('retouch.placeholderReady') : t('retouch.placeholderClickFirst')}
        ...
        <div className="flex flex-wrap items-center justify-center gap-3 mt-6">
            ...
            >
                <RedoIcon className="w-5 h-5 mr-2" />{t('actions.redo')}
            </button>
            ...
            {canUndo && (
              <button 
                  ...
                  aria-label={t('aria.copare')}

              >
                  <EyeIcon className="w-5 h-5 mr-2" />
                  {t('actions.compare')}</button>
            )}
